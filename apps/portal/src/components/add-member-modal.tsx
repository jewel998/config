...
interface AddMemberModalProps {
  ...
  onAddUser: (uid: string, role: RBACRole) => void;
    ...
    >
      <div className="space-y-5">
        ...
        <div className="space-y-2">
          <label className="text-sm font-medium"><Trans>Email address</Trans></label>
        ...
        {foundUser && !isLoading && (
          <div className="rounded-lg border p-3 space-y-3">
            ...
            {isAlreadyMember ? (
              <p className="text-xs text-amber-600"><Trans>This user is already a member of this project.</Trans></p>
            ) : (
        ...
        {!foundUser && !isLoading && email.includes("@") && email.length > 3 && (
          <div className="rounded-lg border border-dashed p-3 space-y-3">
            ...
            {isAlreadyInvited ? (
              <p className="text-xs text-amber-600"><Trans>An invite already exists for this email.</Trans></p>
            ) : (
              <div className="flex items-center gap-2">
                <Select value={role} onValueChange={(v) => setRole(v as RBACRole)}>
                  <SelectTrigger className="w-32 h-8 text-xs"><SelectValue /></SelectTrigger>
