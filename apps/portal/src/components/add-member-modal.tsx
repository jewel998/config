...
export const AddMemberModal = ({
  ...
  const isAlreadyMember =
    foundUser && existingMemberUids.includes(foundUser.uid);
  ...
  const handleAdd = () => {
    ...
    onAddUser(
      foundUser.uid,
      role,
      foundUser.displayName ?? foundUser.email ?? undefined,
    );
    setEmail("");
    onOpenChange(false);
  };
  ...
  return (
    <ResponsiveModal
      ...
      description={
        <Trans>
          Search by email to add an existing user, or send an invite.
        </Trans>
      }
    >
          ...
          <div className="rounded-lg border p-3 space-y-3">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-sm font-medium">
                  {foundUser.displayName ?? foundUser.uid}
                </p>
            ...
            {isAlreadyMember ? (
              ...
              <div className="flex items-center gap-2">
                <Select
                  value={role}
                  onValueChange={(v) => setRole(v as RBACRole)}
                >
          ...
          email.length > 3 && (
            <div className="rounded-lg border border-dashed p-3 space-y-3">
              <p className="text-sm text-muted-foreground">
                <Trans>
                  No user found with this email. Send them an invite?
                </Trans>
              </p>
              {isAlreadyInvited ? (
                <p className="text-xs text-amber-600">
