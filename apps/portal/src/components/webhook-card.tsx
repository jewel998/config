  ...
  return (
    <div className="rounded-lg border p-3 space-y-2">
      ...
      <div className="flex items-center gap-2">
        <button type="button" className="shrink-0" onClick={() => setExpanded(!expanded)}>
          {expanded ? (
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-xs text-muted-foreground font-mono truncate">{maskedUrl}</p>
        </div>
        ...
        {isAdmin && (
          <div className="flex items-center gap-1 shrink-0">
            ...
            <Button
              variant="ghost"
              size="icon-xs"
              onClick={() => testWebhook.mutate({ projectId, webhookId: webhook.id })}
              aria-label={t`Test`}
              disabled={testWebhook.isPending}
            >
              <FlaskConical className="h-3.5 w-3.5" />
            </Button>
            <Button variant="ghost" size="icon-xs" onClick={onEdit} aria-label={t`Edit`}>
              <Pencil className="h-3.5 w-3.5" />
            </Button>
            <Button variant="ghost" size="icon-xs" onClick={onDelete} aria-label={t`Delete`}>
      ...
      {(webhook.eventTypes.length > 0 ||
        ...
        <div className="flex flex-wrap gap-1">
          {webhook.eventTypes.map((e) => (
            <Badge key={e} variant="secondary" className="text-[10px] px-1.5 py-0 rounded-full">
              {e}
            </Badge>
          ))}
          {webhook.resourceCategories.map((c) => (
            <Badge key={c} variant="secondary" className="text-[10px] px-1.5 py-0 rounded-full">
              {c}
            </Badge>
          ))}
