 ...
 */
export const retryFailedRows = onCall({ maxInstances: MAX_INSTANCES }, async (request) => {
  // ─── Auth Check ──────────────────────────────────────────
  if (!request.auth) {
  ...
  if (!data.projectId || !data.jobId) {
    throw new HttpsError("invalid-argument", "projectId and jobId are required");
  }
  ...
  const envRef = projectRef.collection("environments").doc(jobData.environmentId);
